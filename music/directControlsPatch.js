...
if (!MusicManager.prototype.__deathFastControlsPatched) {
  ...
  // Ignore the Idle event generated by an explicit stop while a replacement
  // stream is being prepared. The replacement startTrack owns that transition.
  ...
  MusicManager.prototype.skip = async function fastSkip(guildId) {
    ...
    // Mark the transition before stop(), then clear the old resource. The
    // next track is selected synchronously so the panel changes immediately.
    state.transitioning = true;
  ...
  console.log("⚡ DEATH instant controls loaded: background transitions + guarded skip + live panel updates.");
}
