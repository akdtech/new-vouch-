...
function scoreTrack(track, query, manager) {
  ...
  if (author === q) score += 2500;
  else if (author.includes(q)) score += 1800;
  for (const word of wanted) {
    if (wordInText(word, title)) score += 700;
    if (wordInText(word, author)) score += 500;
  }
...
Module._load = function(request, parent, isMain) {
  ...
  if (!patched && typeof exported === "function" && /(^|[\\/])music[\\/]MusicManager$/.test(request)) {
    ...
    const originalSearch = Original.prototype.search;
    ...
    Original.prototype.search = async function(query, requester = null) {
      ...
      if (isSpotifyUrl(query)) {
        ...
        if (!result?.tracks?.length) throw new Error(`Could not find a playable mirror for Spotify track: ${mirrorQuery}`);
      ...
      for (const prefix of ["ytmsearch:", "ytsearch:", "scsearch:"]) {
        for (const variant of variants) {
          try {
            ...
            console.warn(`⚠️ ${prefix}${variant} failed:`, error?.message || error);
          }
        }
      }
      ...
      const ranked = candidates.map((entry, index) => ({
        ...entry,
        score: scoreTrack(entry.track, query, this) + (entry.source === "ytmsearch:" ? 300 : entry.source === "ytsearch:" ? 200 : 0),
        index
      })).sort((a, b) => b.score - a.score || a.index - b.index);

      const valid = ranked.filter(entry => reliableMatch(entry.track, query, this));
      if (!valid.length) {
        console.warn(`❌ No reliable match for: "${query}"`);
    ...
    Original.prototype.play = async function(args) {
      ...
      if (result?.track) {
        state.autoplayContext = { query: clean(args?.query), artist: clean(this.getTrackAuthor(result.track)), title: clean(this.getTrackTitle(result.track)) };
    ...
    const wrapControl = (name, fallback) => {
      ...
      Original.prototype[name] = async function(guildId, ...rest) {
        ...
        try {
          if (old) {
            result = await old.call(this, guildId, ...rest);
            worked = true;
          }
        } catch (error) {
          console.warn(`⚠️ ${name} fallback:`, error?.message || error);
        }
    ...
    wrapControl("pause", async function(player) {
      if (!player.queue?.current) return false;
      await player.pause(true);
      return true;
    });
    wrapControl("resume", async function(player) {
      if (!player.queue?.current) return false;
      await player.pause(false);
      return true;
    });
    wrapControl("skip", async function(player) {
      if (!player.queue?.current) return false;
      await player.skip();
      return true;
    });
    wrapControl("stop", async function(player, guildId) {
    ...
    wrapControl("shuffle", async function(player) {
      if (typeof player.queue?.shuffle === "function") player.queue.shuffle();
      return true;
    });
    wrapControl("setVolume", async function(player, guildId, volume) {
    ...
    Original.prototype.refreshPanel = async function(guildId) {
      ...
      if (originalRefreshPanel) {
        try { return await originalRefreshPanel.call(this, guildId); }
        catch (error) { console.warn("⚠️ Panel refresh failed:", error?.message || error); }
      }
      return false;
    };
      ...
      try {
        ...
        for (const seed of seeds) {
          ...
          const candidates = (result?.tracks || []).filter(track => {
            ...
            const len = Number(track?.info?.length || track?.length || 0);
    ...
    console.log("🛠️ DEATH Music runtime repair loaded.");
  }
