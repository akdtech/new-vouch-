 ...
 *
 * Fixes the two production failures that were visible in Railway:
 * 1) malformed searches such as ytsearch:ytmsearch:risk it all;
 * 2) concurrent 24/7 player creation causing Discord voice sessions
 *    to race and fail with a missing session id.
 */
...
const STOP = new Set([
  "the","a","an","and","or","of","to","for","with","it","is","in","on","at","by","from",
  "official","music","song","songs","video","audio","lyrics","lyric","full","hd","4k","original",
  "officialaudio","topic","vevo","version","edit","remix","live"
]);
...
function wordMatch(word, text) { return tokens(text).some(t => fuzzy(word, t)); }
...
function artistMatch(wanted, actual) {
  ...
  const words = meaningful(w);
  return words.length > 0 && words.every(x => wordMatch(x, a));
}

function trackLength(track) { return Number(track?.info?.length || track?.length || 0); }
...
function isBadVideo(track, manager) {
  ...
  return /(podcast|lecture|documentary|rabbit hole|compilation|full movie|\bmovie\b|tutorial|how to|reaction|commentary|gameplay|\b\d+\s*hours?\b|10 hour|8 hour|nonstop|medley|karaoke version)/i.test(title);
}
...
function scoreTrack(track, query, manager, source) {
  ...
  if (trackLength(track) > 0 && trackLength(track) <= 8 * 60 * 1000) score += 150;
  return score;
}
...
function looksRelevant(track, query, manager) {
  ...
  const titleHits = wanted.filter(w => wordMatch(w, title)).length;
  const authorHits = wanted.filter(w => wordMatch(w, author)).length;

  if (wanted.length >= 2) {
    return titleHits >= Math.max(1, wanted.length - 1) || titleHits + authorHits >= wanted.length;
  }
...
function trackId(manager, track) {
  return manager.getTrackId(track) || `${manager.getTrackTitle(track)}|${manager.getTrackAuthor(track)}`;
}
...
async function sourceSearch(manager, query, source, requester) {
  /* Critical: use Kazagumo's source option instead of embedding the
     source prefix in the query. Otherwise Kazagumo prepends ytsearch:
     and Lavalink receives ytsearch:ytmsearch:... */
  return manager.kazagumo.search(query, { requester, source });
}
...
Module._load = function(request, parent, isMain) {
  ...
  if (
    ...
    const originalCreatePlayer = Original.prototype.createPlayer;
    ...
    Original.prototype.search = async function(query, requester = null) {
      ...
      if (isSpotifyUrl(query)) {
        ...
        if (!meta) throw new Error("Spotify link could not be read. Please use a Spotify track URL.");
        const result = await this.search(`${meta.title} ${meta.author}`.trim(), requester);
        if (!result?.tracks?.length) {
          throw new Error(`Could not find a playable source for Spotify track: ${meta.title} — ${meta.author}`);
        }
      ...
      if (isYouTubeUrl(query) || isSoundCloudUrl(query)) {
        try {
          ...
          console.warn("⚠️ Direct music URL failed:", error?.message || error);
        }
        return null;
      }
      ...
      for (const source of sources) {
        for (const variant of variants) {
          try {
            const result = await sourceSearch(this, variant, source, requester);
            for (const track of result?.tracks || []) {
              const id = trackId(this, track);
              if (seen.has(id)) continue;
              seen.add(id);
              candidates.push({ track, source, score: scoreTrack(track, query, this, source) });
            }
          } catch (error) {
            console.warn(`⚠️ ${source}${variant} failed:`, error?.message || error);
          }
        }
      }
      ...
      const valid = candidates.filter(item => looksRelevant(item.track, query, this));
      const ranked = valid.length ? valid : candidates.filter(item => !isBadVideo(item.track, this));
      const best = ranked[0];

      if (!best) return { tracks: [], type: "SEARCH_RESULT" };

      console.log(`🔎 Music match: "${query}" → ${this.getTrackTitle(best.track)} — ${this.getTrackAuthor(best.track)} [${best.source}] score=${best.score}`);
    ...
    /* PLAYER CREATION: protect against stale/destroyed players. */
    Original.prototype.createPlayer = async function(guildId, voiceId, textId = voiceId) {
      const existing = this.getPlayer(guildId);
      if (existing?.destroyed) {
        this.players.delete(guildId);
        try { await existing.destroy(); } catch {}
      }
      return originalCreatePlayer.call(this, guildId, voiceId, textId);
    };
    ...
    const wrapControl = (name, original, fallback) => {
      Original.prototype[name] = async function(guildId, ...rest) {
        ...
        try {
          ...
          console.warn(`⚠️ ${name} original control failed:`, error?.message || error);
        }
        if (!player) throw new Error("There is no music player in this server.");
    ...
    wrapControl("pause", originalPause, async player => {
      if (!player.queue?.current) return false;
      await player.pause(true);
      return true;
    });
    wrapControl("resume", originalResume, async player => {
      if (!player.queue?.current) return false;
      await player.pause(false);
      return true;
    });
    wrapControl("skip", originalSkip, async player => {
      if (!player.queue?.current) return false;
      await player.skip();
      return true;
    });
    wrapControl("stop", originalStop, async function(player, guildId) {
      if (typeof player.queue?.clear === "function") player.queue.clear();
      if (typeof player.stop === "function") await player.stop();
      else if (typeof player.stopTrack === "function") await player.stopTrack();
      const state = this.getState(guildId);
      state.autoplayContext = null;
      state.autoplayGeneration = (state.autoplayGeneration || 0) + 1;
      return true;
    });
    wrapControl("shuffle", originalShuffle, async player => {
      if (typeof player.queue?.shuffle === "function") player.queue.shuffle();
      return true;
    });
    wrapControl("setVolume", originalSetVolume, async function(player, guildId, volume) {
      const level = Math.max(1, Math.min(100, Number(volume) || 70));
    ...
    Original.prototype.autoplayNext = async function(guildId, player = this.getPlayer(guildId)) {
      ...
      try {
        const seeds = artist ? [`${artist} official songs`, `${artist} songs`, artist] : [query, `${query} official song`];

        for (const seed of seeds) {
          for (const source of ["ytmsearch:", "ytsearch:", "scsearch:"]) {
            try {
              const result = await sourceSearch(this, seed, source, this.client.user);
              const candidates = (result?.tracks || [])
                .filter(track => {
                  const id = trackId(this, track);
                  if (!id || recent.includes(id) || isBadVideo(track, this)) return false;
                  if (artist) return artistMatch(artist, this.getTrackAuthor(track));
                  return looksRelevant(track, query, this);
                })
                .sort((a, b) => scoreTrack(b, seed, this, source) - scoreTrack(a, seed, this, source));

              const chosen = candidates[0];
              if (!chosen) continue;

              if ((state.autoplayGeneration || 0) !== generation || player.playing || player.paused || player.queue?.current || (player.queue?.length || 0) > 0) return false;
              ...
              console.warn(`⚠️ Context autoplay ${source}${seed} failed:`, error?.message || error);
            }
          }
        }
        return false;
    ...
    Original.prototype.startRecoveryLoop = function() {
      ...
      this.recoveryTimer = setInterval(async () => {
        try {
          ...
          const inVoice = me?.voice?.channelId === this.musicVoiceChannelId;
          if (!player || player.destroyed || !inVoice) await this.ensure247(guildId);
        } catch (error) {
          console.warn("⚠️ Music recovery check failed:", error?.message || error);
        }
      }, 30000);
    ...
    /* CRITICAL: only one voice-player creation may run at a time per guild. */
    Original.prototype.ensure247 = async function(guildId) {
      ...
      const existingLock = this._ensure247Locks.get(guildId);
