...
function normalizeArtist(value) {
  return clean(value).replace(/\s*-\s*topic$/i, "").replace(/\s*\|\s*topic$/i, "").replace(/\s*official$/i, "").replace(/\s*vevo$/i, "").trim();
}
function isGenericArtist(artist) {
  const value = normalizeArtist(artist).toLowerCase();
  return !value || ["unknown artist", "unknown", "various artists", "various", "youtube", "youtube music", "topic"].includes(value);
}
function artistMatches(actual, wanted) {
  ...
  return a.split(/\s*(?:,|&|feat\.?|ft\.?|x)\s*/i).some(part => part === w || part.includes(w) || w.includes(part));
}
...
function fuzzyTokenMatches(token, words) {
  ...
  return words.some(word => {
    ...
    return Math.max(token.length, word.length) >= 4 && levenshtein(token, word) <= maxDistance;
  });
}
function titleLooksRelevant(title, artist) {
  ...
  return words.filter(word => t.includes(word)).length >= Math.max(1, Math.ceil(words.length * 0.5));
}
const STOP_WORDS = new Set(["the", "a", "an", "and", "or", "of", "to", "for", "with", "it", "is", "in", "on", "at", "by", "from", "official", "music", "song", "songs", "video", "audio", "lyrics", "lyric", "full", "hd", "4k", "remix", "live", "version", "original"]);
function queryTokens(query) { return normalize(query).split(" ").filter(token => token.length >= 2 && !STOP_WORDS.has(token)); }
...
function isBadVideo(track, manager) {
  ...
  if (/(podcast|lecture|documentary|rabbit hole|compilation|full movie|movie|mix \d+ hour|\b\d+ hours?\b|tutorial|how to|reaction|commentary|gameplay)/i.test(lower)) return true;
  return false;
}
function scoreTrack(track, query, manager) {
  ...
  score += titleHits * 140 + authorHits * 160;
...
function isValidSearchMatch(track, query, manager) {
  ...
  if (tokens.length >= 2) return tokens.every(token => fuzzyTokenMatches(token, titleWords) || fuzzyTokenMatches(token, authorWords));
  return tokens.length === 1 && (fuzzyTokenMatches(tokens[0], titleWords) || fuzzyTokenMatches(tokens[0], authorWords));
}
function trackId(manager, track) { return manager.getTrackId(track); }
function autoplayCandidate(track, manager, artist, query) {
  if (!track || isBadVideo(track, manager)) return false;
  if (artist) return artistMatches(manager.getTrackAuthor(track), artist) || titleLooksRelevant(manager.getTrackTitle(track), artist);
  if (query) return isValidSearchMatch(track, query, manager);
  return false;
}
...
Module._load = function(request, parent, isMain) {
  ...
  if (!patched && typeof exported === "function" && /(^|[\\/])music[\\/]MusicManager$/.test(request)) {
    ...
    exported.prototype.ensure247 = async function(guildId) {
      ...
      finally { if (this._ensure247Locks.get(guildId) === promise) this._ensure247Locks.delete(guildId); }
    };
    ...
    exported.prototype.search = async function(query, requester = null) {
      ...
      const identifiers = [];
      for (const variant of variants) identifiers.push(`ytmsearch:${variant}`, `ytsearch:${variant}`);
      ...
      for (const identifier of identifiers) {
        try {
          ...
          for (const track of result.tracks) {
            ...
            seen.add(key); allTracks.push(track);
          }
        } catch (error) { console.warn(`⚠️ Validated music search failed for ${identifier}:`, error?.message || error); }
      }
      if (!allTracks.length) return { tracks: [], type: "SEARCH_RESULT" };
      const ranked = allTracks.map((track, index) => ({ track, score: scoreTrack(track, query, this), index })).sort((a, b) => b.score - a.score || a.index - b.index);
      const valid = ranked.filter(item => isValidSearchMatch(item.track, query, this));
      const best = valid[0];
      if (!best) { console.warn(`❌ No reliable music match for: "${query}"`); return { tracks: [], type: "SEARCH_RESULT" }; }
      console.log(`🔎 Validated search: "${query}" → ${this.getTrackTitle(best.track)} — ${this.getTrackAuthor(best.track)} (score ${best.score})`);
      return { tracks: [best.track, ...ranked.filter(item => item !== best && isValidSearchMatch(item.track, query, this)).map(item => item.track)], type: "SEARCH_RESULT" };
    };
    ...
    exported.prototype.play = async function(args) {
      ...
      if (state) {
        ...
        state.autoplayContext = { query, artist: isGenericArtist(artist) ? "" : artist, title: clean(track ? this.getTrackTitle(track) : "") };
      }
      return result;
    };
    ...
    exported.prototype.autoplayNext = async function(guildId, player = this.getPlayer(guildId)) {
      ...
      try {
        ...
        const seeds = artist ? [`${artist} official songs`, `${artist} songs`, artist] : query ? [`${query} official song`, `${query} music`] : [];
        let chosen = null;
          ...
          for (const identifier of [`ytmsearch:${seed}`, `ytsearch:${seed}`]) {
            try {
              ...
              const candidates = result.tracks.filter(track => {
                const id = trackId(this, track);
                return id && !recent.includes(id) && autoplayCandidate(track, this, artist, query);
              });
              if (!candidates.length) continue;
              candidates.sort((a, b) => scoreTrack(b, artist || query || "", this) - scoreTrack(a, artist || query || "", this));
              chosen = candidates[0];
              break;
            } catch (error) { console.warn(`⚠️ Context autoplay search failed for ${identifier}:`, error?.message || error); }
          }
          if (chosen) break;
        }
        ...
        return true;
      } catch (error) { console.error("❌ Context autoplay error:", error?.message || error); return false; }
